...
export default function ContentImageBlock({ content }: ContentImageBlockProps) {
  ...
  const title = content?.title || (lang === 'da' ? 'Overskrift her' : 'Title here')
  const description = content?.description || (lang === 'da' ? 'Beskrivelse her...' : 'Description here...')
  const buttonText = content?.buttonText || (lang === 'da' ? 'Læs mere' : 'Read more')
